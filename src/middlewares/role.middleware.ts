...
//  * @param {boolean} [options.strict] - If true, only exact role matches are allowed.
...
//  */
// function roleMiddleware({ roles, strict = true }: RoleMiddlewareOptions) {
...
//  */
// async function getFilteredRoleHierarchy(roleName: string, minRole: string): Promise<string[]> {
...
//  */
// async function getRoleHierarchy(roleName: string): Promise<string[]> {
//   const roleHierarchy = await db.roles.findUnique({
//     where: { name: roleName },
//     include: {
//       parent: true,
//     },
